...
from vit_dataset import TestImageDataset
from vit_model import PatchCoreViT
import vit_config
from vit_heatmap import make_and_save_heatmap
import cv2
...
def test_patchcore():
    ...
    ckpt_path = vit_config.MODEL_PATH
    ...
    good_ds = TestImageDataset(vit_config.TEST_GOOD_FOLDER)
    defect_ds = TestImageDataset(vit_config.TEST_DEFECT_FOLDER)
    ...
    for i, img in enumerate(tqdm(good_ds)):
        ...
        save_path = vit_config.OUT_PATH_GOOD + "/" + str(i) + ".bmp"
    ...
    for i, img in enumerate(tqdm(defect_ds)):
        ...
        save_path = vit_config.OUT_PATH_DEF + "/" + str(i) + ".bmp"
