...
def make_and_save_heatmap(img_tensor: torch.Tensor, patch_scores: torch.Tensor, fmap_h: int, fmap_w: int, save_path: str = "heatmap.bmp"):
    ...
    heatmap_smooth = cv2.GaussianBlur(heat, ksize=(3, 3), sigmaX=0.5)

    heat_up = cv2.resize(heatmap_smooth.astype(np.float32), (W_img, H_img), interpolation=cv2.INTER_CUBIC)
