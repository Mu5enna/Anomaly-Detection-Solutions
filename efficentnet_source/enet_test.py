...
from enet_dataset import TestImageDataset
from enet_model import PatchCoreEffB3
import enet_config
from enet_heatmap import make_and_save_heatmap
import cv2
...
def test_patchcore():
    ...
    ckpt_path = enet_config.MODEL_PATH
    ...
    good_ds = TestImageDataset(enet_config.TEST_GOOD_FOLDER)
    defect_ds = TestImageDataset(enet_config.TEST_DEFECT_FOLDER)
    ...
    for i, img in enumerate(tqdm(good_ds)):
        ...
        save_path = enet_config.OUT_PATH_GOOD + "/" + str(i) + ".bmp"
    ...
    for i, img in enumerate(tqdm(defect_ds)):
        ...
        save_path = enet_config.OUT_PATH_DEF + "/" + str(i) + ".bmp"
